...
public record AccessControl(
    @JsonProperty("public") Boolean isPublic   // 是否公开（唯一职责：访问控制）
) {
...
public record CustomMetadata(
    Map<String, String> customMetadata          // 自定义元数据（键值均为字符串）
) {
...
public record FileRequest(
    ...
    // 通过 @JsonUnwrapped 组合 VO
    @JsonUnwrapped AccessControl access,       // 访问控制
    @JsonUnwrapped FileTags fileTags,          // 文件标签
    @JsonUnwrapped CustomMetadata metadata     // 自定义元数据
) {}
```
