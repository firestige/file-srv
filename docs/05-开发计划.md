...
| 模块                           | 状态      | 完成内容                                                                             |
| ------------------------------ | --------- | ------------------------------------------------------------------------------------ |
| **file-srv-core**              | ⏳ 进行中 | DDD 分层结构、Controller 骨架、Service 接口、Registry 实现                           |
| **file-srv-common**            | ✅ 完成   | 常量、异常体系、Result 包装、TaskContext、**SPI 契约 (StorageAdapter/SharedPlugin)** |
| **file-srv-adapters/hcs**      | ✅ 完成   | HcsObsAdapter 完整实现（含分片）、自动配置，依赖 common                              |
| **file-srv-aspect**            | ⏳ 待开始 | 模块框架已建立                                                                       |
| **file-srv-autoconfiguration** | ⏳ 进行中 | 基础配置类                                                                           |
| **file-srv-plugins**           | ✅ 完成   | 插件实现（实现 common 中的 SharedPlugin SPI）                                        |
...
| 里程碑 | 目标                                  | 迭代     |
| ------ | ------------------------------------- | -------- |
| **M1** | 同步上传/下载可用，HcsObsAdapter 完成 | 迭代 1   |
| **M2** | 分片上传全链路 + 插件框架             | 迭代 2   |
| **M3** | 查询优化、缓存、可观测性              | 迭代 3-4 |
| **M4** | 多适配器支持、生产就绪                | 迭代 5   |
