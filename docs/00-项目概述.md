...
- **多存储支持**：支持公有云对象存储（华为云OBS、阿里云OSS、腾讯云COS、AWS S3）、私有化部署存储（MinIO、Ceph）、网络存储（NAS）
...
| 对象存储SDK | 华为云 OBS SDK | 3.23.9 | 华为云存储适配 |
...
```
icc-file-srv/
├── file-srv-core/                   # 核心模块（领域层 + 应用层 + 基础设施接口）
...
│       └── infra/                   # 基础设施接口
│           ├── storage/             # 存储适配器接口（StorageAdapter）
│           └── plugin/              # 插件接口（SharedPlugin）
│
...
| 模块 | 职责 | 依赖关系 |
|-----|------|---------|
| **file-srv-core** | 核心业务逻辑，包含领域模型、应用服务、Controller、基础设施接口定义 | 依赖 common |
| **file-srv-common** | 无依赖的公共工具、常量、异常、共享上下文（如 TaskContext） | 无依赖 |
| **file-srv-adapters** | 存储适配器实现集合，每种存储一个子模块 | 依赖 core（实现接口） |
| **file-srv-aspect** | 切面管理（可见性控制、审计日志等横切关注点） | 依赖 core |
| **file-srv-autoconfiguration** | Spring Boot 自动装配配置 | 依赖 core |
| **file-srv-plugins** | 后处理插件集合，上传完成后按顺序调用 | 依赖 core（实现 SharedPlugin） |
| **file-srv-test** | 集成测试、基准性能测试 | 依赖 autoconfiguration |

## 核心特性

### 1. 文件上传
- **同步上传**：适用于≤10MB的小文件，单次HTTP请求完成
- **分片上传**：适用于≥5MB的大文件，支持断点续传
  - 分片数量上限：10,000片
...
### 6. 文件唯一标识 (fkey)
- **用户指定**：客户端提供业务相关的唯一标识
- **对象存储生成**：使用对象存储的对象键（如OBS对象名）
- **分布式ID**：非对象存储场景使用Snowflake等算法
...
- [02-API设计](./02-API设计.md) - REST API 接口规范
