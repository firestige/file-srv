...
> 目标：在迭代1的同步小文件闭环基础上，补齐大文件分片上传（OBS），实现可恢复的分片任务生命周期、元数据与任务状态持久化，并提供对外 API。设计粒度可直接驱动编码。
...
### 3.2 DTO
- `FileMetadata`（沿用迭代1）：filename, size, contentType, creator, tags, bucket, objectKey(optional)。
...
### 3.3 对外接口说明
- 不新增 Restful API 路由，复用现有 `/api/v1/files/...` 体系。迭代2仅扩展内部接口（Handler/Service/Adapter）以支撑分片能力。
...
- 任务超时：默认 24h 过期；过期后后台可定时清理（迭代2实现手动 abort，定时清理可选）。
- 幂等性：
