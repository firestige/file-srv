...
> 迭代 1 仅实现上述方法；查询/分页留空实现或 `UnsupportedOperationException`。
...
- `FilesController`
  ...
  - POST `/api/v1/files/metadata` → 迭代 1 可返回 `Result.failure(501, "Not implemented")`。
...
> 迭代 1 仅同步上传，分片接口返回 UnsupportedOperationException。但预留状态机，迭代 2 实现。
